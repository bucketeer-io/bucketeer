...
import {
  invalidatePushDetails,
  useQueryPushDetails
} from '@queries/push-details';
...
const EditPushModal = ({ isOpen, onClose }: EditPushModalProps) => {
  ...
  const { id: pushId, state, errorToast } = useActionWithURL({});
  ...
  const { data: collection, error: environmentError } = useFetchEnvironments({
  ...
  const push = pushCollection?.push;
  ...
  const onSubmit: SubmitHandler<EditPushForm> = async values => {
    ...
    if (resp) {
      ...
      invalidatePushDetails(queryClient, {
        id: push?.id || pushId || '',
        environmentId
      });
      onClose();
  ...
  useEffect(() => {
    if (pushError || environmentError || tagError)
      errorToast((pushError || environmentError || tagError) as Error);
  }, [pushError, environmentError, tagError]);
