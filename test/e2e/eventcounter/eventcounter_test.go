...
func TestGoalBatchEventCounter(t *testing.T) {
	t.Parallel()
	featureClient := newFeatureClient(t)
	defer featureClient.Close()
	experimentClient := newExperimentClient(t)
	...
	for i := 0; i < retryTimes; i++ {
		...
		if vcA.ValueSumPerUserMean != float64(0.3) {
			continue
		}
		if vcA.ValueSumPerUserVariance != float64(0) {
			continue
...
func registerGoalBatchEvent(t *testing.T, tag, goalID, userID string) {
	t.Helper()
	c := newGatewayClient(t)
	defer c.Close()
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	gb, err := ptypes.MarshalAny(&eventproto.GoalBatchEvent{
		UserId: userID,
	...
	if len(response.Errors) > 0 {
		t.Fatalf("Failed to register a goal batch event. Error: %v", response.Errors)
	}
}
