...
/**
 * `useData` implementations return a fresh data object on every call (plain
 * object literals, not memoized), including fresh `getLabel`/`loadMore`/
 * `onSearchChange` closures every render regardless of whether the data they
 * close over actually changed. Comparing by reference would always see a
 * change and update on every render forever, so compare only the fields
 * that determine what's rendered (loading flags and `options` by value) and
 * ignore function identity - callers always read functions off the latest
 * stored object, so it's fine for them to lag one state update behind an
 * options-only change.
 */
function isFieldDataRenderEquivalent(
  previous: FilterFieldData | undefined,
  next: FilterFieldData
): boolean {
  if (!previous) return false;

  // Ensure options are valid arrays
  if (!Array.isArray(previous.options) || !Array.isArray(next.options)) {
    return false;
  }
  ...
  return previous.options.every(
    (option, index) =>
      option.value === next.options[index].value &&
      option.label === next.options[index].label
  );
}

/**
 * Mounts once per field and calls that field's `useData` unconditionally,
 * so each field's hooks (e.g. useAuth, useQueryTags) run in their own
 * component instance rather than in a loop inside `FilterModal`.
 */
function FieldDataLoader<F>({
  field,
  enabled,
  value,
  onData
  ...
  enabled: boolean;
  value: FilterOption['filterValue'] | undefined;
  onData: (type: string, data: FilterFieldData) => void;
}) {
  const data = field.useData
    ? field.useData({ enabled, value })
    : { options: [] };

  useEffect(() => {
    onData(field.type, data);
  });
...
function FilterModal<F>({
  ...
  const [fieldData, setFieldData] = useState<Record<string, FilterFieldData>>(
    {}
  );

  const handleFieldData = useCallback((type: string, data: FilterFieldData) => {
    setFieldData(prev =>
      isFieldDataRenderEquivalent(prev[type], data)
        ? prev
        : { ...prev, [type]: data }
    );
  }, []);
