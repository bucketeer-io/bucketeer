...
func TestTemplateRenderer_RenderPasswordChangedEmail(t *testing.T) {
	t.Parallel()
	...
	subject, body := renderer.RenderPasswordChangedEmail("en", "test@example.com")
...
func TestTemplateRenderer_RenderPasswordSetupEmail(t *testing.T) {
	t.Parallel()
	...
	subject, body := renderer.RenderPasswordSetupEmail("en", setupURL, ttl, "test@example.com")

	if subject == "" {
		t.Error("expected non-empty subject")
	}

	if !strings.Contains(body, setupURL) {
		t.Error("expected body to contain setup URL")
	}
...
func TestTemplateRenderer_RenderPasswordResetEmail(t *testing.T) {
	...
	subject, body := renderer.RenderPasswordResetEmail("en", resetURL, ttl, "test@example.com")
	...
	if !strings.Contains(body, resetURL) {
		t.Error("expected body to contain reset URL")
	}
...
func TestTemplateRenderer_CustomTemplates(t *testing.T) {
	t.Parallel()
		...
		Templates: TemplatesConfig{
			En: TemplatesByLanguage{
				PasswordChanged: Template{
					Subject: "Custom Password Changed Subject",
					Body:    "Custom body for {{userEmail}}",
				},
			},
		},
	}
	...
	if subject != "Custom Password Changed Subject" {
		t.Errorf("expected custom subject, got: %s", subject)
	}

	if !strings.Contains(body, "Custom body for test@example.com") {
		t.Errorf("expected custom body, got: %s", body)
	}
}

func TestTemplateRenderer_DefaultTemplates(t *testing.T) {
	t.Parallel()
	...
	// Test that default templates are used when custom templates are not provided
	subject, body := renderer.RenderPasswordChangedEmail("en", "test@example.com")

	if subject != defaultPasswordChangedSubject {
