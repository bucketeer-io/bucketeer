      ...
      <div className="typo-para-medium text-gray-700">
        {changeType === 'new-rule' ? (
          ...
          <div className="inline items-center">
            ...
            <strong>{label}</strong>
            <StrategyList variations={variations} t={t} className="inline" />
          </div>
        )}
      </div>
    </div>
              ...
              components={{
                ...
                variantElement: (
                  <div className="inline-flex items-center gap-x-1 ml-1">
                    ...
                    <p className="inline max-w-[220px] truncate">
                ...
                variantElement: (
                  <div className="inline flex-wrap">
                    {audienceIncluded?.map((audience, index) => (
                      <div
                        key={index}
                        className="inline-flex items-center gap-x-1 ml-1"
                      >
                        <div className="inline-flex flex-center size-fit">
                        ...
                        <div className="inline-flex">
                          ...
                          {index !== audienceIncluded.length - 1 &&
                            t('common:comma')}
                        </div>
                      </div>
                    ))}
...
const StrategyList = ({
  ...
  !!variations?.length && (
    <div
      className={cn(
        'mt-1 flex flex-wrap gap-1 items-center gap-x-1',
        className
      )}
    >
      {variations.map((v, index) => (
        <div key={index} className="inline-flex items-center gap-x-1 ml-1">
          <div className="flex-center size-fit">
            <FlagVariationPolygon index={v.variationIndex || 0} />
          </div>
          <p className="max-w-[220px] h-full truncate">{v.variation}</p>
          {!isNil(v.weight) && (
            <p className="text-gray-700">
              <span>
                ({v.weight?.toString()}%)
                {index !== variations.length - 1 && t('common:comma')}
              </span>
            </p>
          )}
        </div>
