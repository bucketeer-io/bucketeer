...
import {
  ...
  hasChangePosition,
  hasSameFeatureTargets,
  hasSamePrerequisites,
...
const TargetingPage = ({
  ...
  const checkEditRule = useCallback(
    (type: RuleCategory, index?: number): boolean => {
      const handleCheckDirty = (obj?: { [key: string]: boolean }) =>
        checkFiledDirty(obj ?? {});
      const isSameSegmentRule = hasSameSegmentRule(
        feature.rules[index || 0],
        segmentRulesWatch[index || 0]
      );
      const isSameDefaultValue = hasSameStrategy(
        feature.defaultStrategy,
        getValues().defaultRule,
        true
      );
      switch (type) {
        case RuleCategory.CUSTOM:
          if (isEmpty(index)) return false;
          return !isSameSegmentRule;

        case RuleCategory.PREREQUISITE:
          return (
            handleCheckDirty(
              dirtyFields.prerequisites as unknown as { [key: string]: boolean }
            ) || !hasSamePrerequisites(prerequisites, feature.prerequisites)
          );
        ...
        case RuleCategory.INDIVIDUAL:
          return (
            ...
            ) || !hasSameFeatureTargets(individualRules, feature.targets)
          );
  ...
  const onDiscardChanges = useCallback(
    (type: DiscardChangesType, index?: number) => {
      const handleResetField = (field: keyof TargetingSchema, data: object) => {
        resetField(field);
        reset({ ...getValues(), [field]: data }, { keepDirty: true });
      };
      if (type === DiscardChangesType.PREREQUISITE) {
        const { prerequisites } = feature;
        const resetPrerequisites = prerequisites.length
          ? handleCreatePrerequisites(feature.prerequisites)
          : [];
        handleResetField('prerequisites', resetPrerequisites);
      }
      if (type === DiscardChangesType.INDIVIDUAL) {
        ...
        const resetIndividualRules = targets.length
          ...
          : [];
        handleResetField('individualRules', resetIndividualRules);
      }
      if (type === DiscardChangesType.CUSTOM && typeof index === 'number') {
      ...
      if (type === DiscardChangesType.DEFAULT) {
        handleResetField(
          'defaultRule',
          handleCreateDefaultValues(feature).defaultRule
        );
      }
