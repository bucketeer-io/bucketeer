...
//
//	mockgen -source=transaction.go -aux_files=github.com/bucketeer-io/bucketeer/pkg/storage/v2/postgres=client.go -package=mock -destination=./mock/transaction.go
//
...
func (m *MockTransaction) ExecContext(ctx context.Context, query string, args ...any) (postgres.Result, error) {
	m.ctrl.T.Helper()
...
func (m *MockTransaction) QueryContext(ctx context.Context, query string, args ...any) (postgres.Rows, error) {
	m.ctrl.T.Helper()
	varargs := []any{ctx, query}
	for _, a := range args {
		varargs = append(varargs, a)
...
func (m *MockTransaction) QueryRowContext(ctx context.Context, query string, args ...any) postgres.Row {
	m.ctrl.T.Helper()
	varargs := []any{ctx, query}
	for _, a := range args {
		varargs = append(varargs, a)
