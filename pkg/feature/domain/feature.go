...
func validateClauses(clauses []*feature.Clause) error {
	...
	for _, c := range clauses {
		...
		if _, ok := ids[c.Id]; ok {
			return errors.New("feature: clause already exists")
		}
		ids[c.Id] = struct{}{}
...
func validateTargets(targets []*feature.Target, variations []*feature.Variation) error {
	for _, t := range targets {
		if _, err := findVariation(t.Variation, variations); err != nil {
			return errors.New("feature: variation not found")
		}
		if len(t.Users) == 0 {
			return errors.New("feature: target must have at least one user")
		}
	}
	return nil
}
