...
func TestNewMonthlySummaryStoragePostgres(t *testing.T) {
	t.Parallel()
	mockController := gomock.NewController(t)
	defer mockController.Finish()
	s := NewMonthlySummaryStorage(mock.NewMockTransaction(mockController))
	assert.IsType(t, &monthlySummaryStorage{}, s)
}

func TestUpsertMonthlySummaryBatchPostgres(t *testing.T) {
	t.Parallel()
	mockController := gomock.NewController(t)
	defer mockController.Finish()
	...
	for _, p := range patterns {
		t.Run(p.desc, func(t *testing.T) {
			s := &monthlySummaryStorage{qe: mock.NewMockTransaction(mockController)}
			if p.setup != nil {
				p.setup(s)
			}
...
func TestListMonthlySummariesPostgres(t *testing.T) {
	t.Parallel()
	mockController := gomock.NewController(t)
	defer mockController.Finish()
