...
function FilterModal<F>({
  ...
  // Call every field's data hook in a stable order. `config.fields` MUST be a
  // constant-length array: each `useData` is a hook and is invoked
  // unconditionally in config order, so hook count stays stable across renders.
  // Each is gated by whether that field is currently selected.
