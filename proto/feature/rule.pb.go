...
// RuleListValue is a wrapper for a repeated Rule field so that update
// requests can distinguish "not set" (field is absent) from
// "replace with an empty list" (field is present with no values).
