...
import { resetPassword, validateResetPassword } from '@api/auth/setup-password';
...
const ResetPassword = () => {
    ...
    async values => {
      try {
        ...
        notify({
          ...
          message: t('message:reset-password-success')
        });
  ...
  useEffect(() => {
    const verifyToken = async () => {
      if (!resetToken) {
        errorNotify('error', t('message:invalid-reset-token-password'));
        return navigate(PAGE_PATH_AUTH_SIGNIN);
      }

      try {
        await validateResetPassword(resetToken);
      } catch (error) {
        errorNotify(error, t('message:invalid-reset-token-password'));
        return navigate(PAGE_PATH_AUTH_SIGNIN);
      }
