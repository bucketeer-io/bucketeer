...
func TestIncrementEnvEvents_Aggregation(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// shouldFailFlush makes PFADD fail first; INCRBY is skipped for that pair (ordering).
			// flushAggregatedCounts still attempts every key pair; this case has one pair only.
			expectedEventCountKeys: 0, // no successful INCRBY; skip key-count assertions on error
			expectedUserCountKeys:  0, // PFADD failed before recording user keys
			expectedFailCount:      1,
			expectFlushCalled:      true,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectFlushCalled {
				...
				// (failed flushes leave partial or empty mock state; counts are not asserted)
...
// TestIncrementEnvEvents_MultiSourceMetricsAttribution must not call t.Parallel():
// it temporarily replaces package-level evaluationEventCounter.
...
func (m *mockPipeClient) PFMerge(dest string, keys ...string) *goredis.StatusCmd {
	m.commands = append(m.commands, "PFMERGE")
