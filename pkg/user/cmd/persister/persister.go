...
func RegisterCommand(r cli.CommandRegistry, p cli.ParentCommand) cli.Command {
	cmd := p.Command(command, "Start user persister")
	persister := &persister{
		CmdClause:     cmd,
		...
		pullerMaxExtension: cmd.Flag(
			"puller-max-extension",
			"Maximum duration in seconds until a message expires.",
		).Default("600s").Duration(),
...
func (p *persister) createPuller(ctx context.Context, logger *zap.Logger) (puller.Puller, error) {
	...
	return client.CreatePuller(*p.subscription, *p.topic,
		...
		pubsub.WithMaxExtension(*p.pullerMaxExtension),
